...
The target had `/boot/initramfs-linux.img` and `linux-lts.preset`, but no `linux.preset` — so
`mkinitcpio -P` silently skipped the primary kernel and its initramfs could no longer be
regenerated.

**What Changed** — the `etc/mkinitcpio.d/linux.preset` entry is removed from `kiro_final`'s
`paths_to_remove` list, replaced by a comment recording why it must not come back.

**Why:** the entry predates the `kiro_kernel` module. It assumed `linux.preset` is always an
archiso live-only artifact and that the real kernel ships a differently-named preset (e.g.
`linux-cachyos.preset`). That assumption breaks on the default `kernel="linux linux-lts"` build,
where `linux` *is* an installed target kernel and `linux.preset` is its real preset.

### Technical Details

Install order is `unpackfs@rootfs` → `kiro_kernel` → `initcpiocfg` → `initcpio` → … → `kiro_final`:

1. `unpackfs` copies the airootfs in, bringing the live `etc/mkinitcpio.d/linux.preset` (210 b).
2. `kiro_kernel` removes the live artifacts **first**, then writes a real preset for every
   detected kernel — including `linux.preset`. Its inline comment already names this hazard;
   the module was correct and is unchanged.
3. `initcpio` runs `mkinitcpio -P`, which built from both presets (`Calamares.log`, 07:59:31:
   `Building image from preset: /etc/mkinitcpio.d/linux.preset`).
4. `kiro_final` then deleted that freshly written preset.

Because `kiro_kernel` has already cleaned up and regenerated by the time `kiro_final` runs, the
entry could only ever delete a legitimate file. `kiro_kernel` returns an error tuple when no
kernel is found, which aborts the install, so no safety-net path needs it — hence a deletion
rather than a new conditional.

Note on diagnosis: a bare-metal box appeared to pass this check, but its `linux.preset` was the
588-byte **stock Arch** file with an mtime *after* Calamares' deletion — Arch's
`90-mkinitcpio-install.hook` had regenerated it during a later kernel-touching transaction. The
defect was present there too; it had simply self-healed.

`etc/mkinitcpio.d/kiro` is still removed by `kiro_kernel` — no kernel is ever named `kiro`.
...
- **`usr/lib/calamares/modules/kiro_bootloader/main.py`** — `install_systemd_boot()`
  now runs `bootctl install` through `subprocess.run(capture_output=True, text=True)`
  instead of `subprocess.call()`, warns with the exit code and stderr when it fails,
  and logs its stdout. A non-zero exit is deliberately **not** fatal on its own:
  new `systemd_boot_is_installed()` checks the ESP for `loader/` and an `.efi` under
  `EFI/systemd/`, and only when that postcondition fails is a
  `subprocess.CalledProcessError` raised. `create_loader()` additionally creates the
  `loader/` directory with `os.makedirs(..., exist_ok=True)` before writing
  `loader.conf`.
- **Why:** a real-hardware install (Intel, NVMe, systemd-boot, ESP on
  ...
  traceback. The failure is now reported at the point it happens and lands in the
  existing `subprocess.CalledProcessError` handler in `run()`, which shows the
  command, the exit code and the captured output to the user.
- **Why gate on the postcondition rather than the exit code:** `bootctl install`
  writes the ESP layout and *then* updates EFI variables, and can fail the second
  having completed the first (read-only efivarfs, quirky firmware, VMs). Those
  installs are bootable and used to succeed; failing them on exit code alone would
  have been a regression. Checking what bootctl actually left on the ESP keeps the
